...
t = text(mean(xlim),33,mp.title);
set(t,'horiz','center','fontsize',16,'fontname','times')

%%
ts = 0.5;
t2(1) = text(-72.8, 37.2,mp.list{1});
set(t2(1),'fontname','times','fontsize',14);
maps = {[0 0 0.5625]; [0.5 1.0 0.5]; [0.5 0 0]}; 
for in = 2:length(mp.list)
    t2(in) = text(-72.8,37,mp.list{in});
    above = get(t2(in-1),'position');
    above(2) = above(2) - ts;
    set(t2(in),'position',above,'color',maps{in-1})
    set(t2(in),'fontname','times','fontsize',14)
end
%%
...
g2(1) = (g(1)+g(3)/2) - g2(3)/2;
 g2(2) = g(2) - 0.04;
g2(4) = g2(4)/2;
set(cbar,'position',g2,'visible','off')
