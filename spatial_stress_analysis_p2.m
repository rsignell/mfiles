...
mean_event_dur = NaN([length(cvalues) length(mlists) size(lon,1) size(lon,2)]);    %mean event duration
mean_quis_dur = NaN([length(cvalues) length(mlists) size(lon,1) size(lon,2)]); %mean duration between events
mean_rec_interval = NaN([length(cvalues) length(mlists) size(lon,1) size(lon,2)]); %time period/events, time period in days
med_event_dur = NaN([length(cvalues) length(mlists) size(lon,1) size(lon,2)]);
med_quis_dur = NaN([length(cvalues) length(mlists) size(lon,1) size(lon,2)]);
perc_over = NaN([length(cvalues) length(mlists) size(lon,1) size(lon,2)]);
ptween = NaN([length(cvalues)-1 length(mlists) size(lon,1) size(lon,2)]);
    ...
    for ii = 1:length(iiL)-1
        ...
        for jj2 = 1:length(jt)
            for ii2 = 1:length(it)
                ...
                ttau = tauwc(:,jj2,ii2);
                for cc = 1:length(cvalues)                     
                    
                    
                    %Storms
                    dStorm = diff([0; ttau >= cvalues(cc); 0]);
                    ...
                    %Quiscent periods
                    dQuis = diff([0; ttau < cvalues(cc); 0]);
